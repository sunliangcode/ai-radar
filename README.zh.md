...
```
你的 Context → 信号 → 逐条评分与摘要 → Today Deck → 决策
```
...
大多数 AI 资讯是噪音。AI Radar 用你的上下文过滤噪音，只留下值得关注的变化，并帮你决定下一步。

- ✓ **个人 Context** — 画像、项目与技术栈，相关性围绕「你」
- ✓ **主语言** — 摘要由 LLM 按中文 / 英文生成；英文标题经本地 **Argos Translate** 侧车译为中文
- ✓ **Today Radar Deck** — 每天最多约三张重大变化；逐张 **关注 / 忽略 / 决策**
- ✓ **Explore** — 全量信号浏览与一句话 AI 总结（原 Feed 在此）
- ✓ **喜欢 / 不喜欢关键词** — 收藏或不感兴趣会写入偏好短语
- ✓ **Changes & Decisions** — 用白话说明为何重要，并跟进待复盘的决策
- ✓ **Radar Chat** — 与已配置模型多轮对话；可从变化详情带 `?changeId=` 切入
- ✓ **AI 监控** — 队列进度与流式 I/O，入口在 **设置 → 系统**
...
1. 打开 **我是谁**（设置 / Context），填写角色、项目与技术栈，并选择**主语言**（中文或英文）。
2. 如需则导入源 Pack，然后 **立即更新** — 抓取会评分与摘要，并重算影响。
3. 在 **今日** Radar Deck 上逐张处理（**关注 / 忽略 / 决策**）。
4. 需要时去 **Explore** 扫全量；打开变化看证据，或从变化进入 **Radar Chat**。
5. 在 **设置 → 系统** 查看分析队列进度，以及本地 Ollama 的流式输入输出。
